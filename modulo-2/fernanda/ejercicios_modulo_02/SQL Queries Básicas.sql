...
/*2 Realiza una consulta que obtenga los teléfonos y direcciones de aquellas empresas 
de la tabla customers que se encuentren en USA (es su país).*/

SELECT phone, address_line1 
FROM customers
WHERE country = "USA";
/*3Realiza una consulta que obtenga los nombres y apellidos de las personas de contacto
...
/*4 Busca aquellos registros de la tabla customers que tengan un valor guardado para el campo state. 
Este atributo solo es valido para ciertos países por lo que habrá varias entradas con valor NULL.*/
SELECT *
FROM customers
WHERE state IS NOT NULL;

-- 6 Busca aquellos registros de la tabla customers que correspondan a clientes de USA pero que no tengan un valor guardado para el campo state.

SELECT *
FROM customers
WHERE country = 'USA' AND (state IS NULL OR state = ''); 
